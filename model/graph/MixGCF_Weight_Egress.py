...
class MixGCF_Weight_Egress(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(MixGCF_Weight_Egress, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['MixGCF_Weight_Egress'])
        self.n_layers = int(args['-n_layer'])
        self.n_negs = int(args['-n_negs'])
        self.weight = int(args['-weight'])
        self.model = MixGCF_Encoder(self.data, self.emb_size, self.n_negs,self.n_layers, self.weight)

        wandb.log({
            "Layer": int(args['-n_layer']),
            "n_negs": int(args['-n_negs'])
        })
...
class MixGCF_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_negs,n_layers, weight):
