...
class SGL_Weight_Egress(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(SGL_Weight_Egress, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['SGL_Weight_Egress'])
        ...
        self.model = SGL_Encoder(self.data, self.emb_size, drop_rate, n_layers, temp, aug_type, self.weight)
        
        wandb.log({
            "CL rate": float(args['-lambda']),
            "Augmentation type": int(args['-augtype']),
            "Drop rate": float(args['-droprate']),
...
class SGL_Encoder(nn.Module):
    def __init__(self, data, emb_size, drop_rate, n_layers, temp, aug_type, weight):
    ...
    def _init_model(self):
        ...
        embedding_dict = nn.ParameterDict({
            'user_emb': nn.Parameter(Egress_dim_initialization(torch.empty(self.data.user_num, self.emb_size))),
            'item_emb': nn.Parameter(Egress_dim_initialization(torch.empty(self.data.item_num, self.emb_size))),
        })
