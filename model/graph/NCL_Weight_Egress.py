...
class NCL_Weight_Egress(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(NCL_Weight_Egress, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['NCL_Weight_Egress'])
        ...
        wandb.log({
            ...
            "Temp": float(args['-tau']),
            "ssl_reg": float(args['-ssl_reg']),
            "hyper_layers": int(args['-hyper_layers']),
            "alpha": float(args['-alpha']),
            "proto_reg": float(args['-proto_reg']),
