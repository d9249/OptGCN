...
class BUIR_Weight_Egress(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(BUIR_Weight_Egress, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['BUIR_Weight_Egress'])
        self.momentum = float(args['-tau'])
        self.n_layers = int(args['-n_layer'])
        self.drop_rate = float(args['-drop_rate'])
        self.weight = int(args['-weight'])
        self.model = BUIR_NB(self.data, self.emb_size, self.momentum, self.n_layers, self.drop_rate, self.weight, True)

        wandb.log({
            "Layer": int(args['-n_layer']),
            "Temp": float(args['-tau']),
            "Drop rate": float(args['-drop_rate'])
        })
...
class BUIR_NB(nn.Module):
    def __init__(self, data, emb_size, momentum, n_layers, drop_rate, weight, drop_flag=False):
        super(BUIR_NB, self).__init__()
        self.emb_size = emb_size
        self.momentum = momentum
        self.weight = weight
        self.online_encoder = LGCN_Encoder(data, emb_size, n_layers, drop_rate, weight, drop_flag)
        self.target_encoder = LGCN_Encoder(data, emb_size, n_layers, drop_rate, weight, drop_flag)
...
class LGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers, drop_rate, weight, drop_flag=False):
    ...
    def _init_model(self):
        def Egress_dim_initialization(tensor):
            stdv = torch.sqrt(1. / torch.tensor(tensor.size(1), dtype=torch.float))
            return tensor.uniform_(-stdv, stdv)
        
        def Egress_user_initialization(tensor):
            stdv = torch.sqrt(1. / torch.tensor(self.data.user_num, dtype=torch.float))
            return tensor.uniform_(-stdv, stdv)
        
        def Egress_item_initialization(tensor):
            stdv = torch.sqrt(1. / torch.tensor(self.data.item_num, dtype=torch.float))
            return tensor.uniform_(-stdv, stdv)

        embedding_dict = nn.ParameterDict({
            'user_emb': nn.Parameter(Egress_dim_initialization(torch.empty(self.data.user_num, self.latent_size))),
            'item_emb': nn.Parameter(Egress_dim_initialization(torch.empty(self.data.item_num, self.latent_size))),
        })
    ...
    def forward(self, inputs):
        ...
        arr = [int(self.weight), int(self.weight), int(self.weight)]
        
        wandb.log({"Weight": arr})

        for k in range(self.layers):
            ego_embeddings = torch.sparse.mm(A_hat, ego_embeddings)
            # all_embeddings += [ego_embeddings]
            all_embeddings.append(arr[k] * ego_embeddings)
