...
class BUIR_Egress(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(BUIR_Egress, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['BUIR_Egress'])
    ...
    def save(self):
        save_path = "./embedding/Egress/BUIR_Egress/"
        self.best_p_u, self.best_u, self.best_p_i, self.best_i = self.model.get_embedding()
        torch.save(self.best_p_u, save_path + 'best_p_u.pt')
        torch.save(self.best_u, save_path + 'best_u.pt')
        torch.save(self.best_p_i, save_path + 'best_p_i.pt')
        torch.save(self.best_i, save_path + 'best_i.pt')
